...
$envVars = @(
    ...
    New-EnvVar 'Smtp__SecureSocketOptions' (Get-Setting -Values $envValues -Name 'SMTP_SECURE_SOCKET_OPTIONS' -Default 'StartTls')
    New-EnvVar 'Smtp__TimeoutSeconds' (Get-Setting -Values $envValues -Name 'SMTP_TIMEOUT_SECONDS' -Default '5')
    ...
    New-EnvVar 'Email__Provider' (Get-Setting -Values $envValues -Name 'EMAIL_PROVIDER' -Default 'GmailApi')
    New-EnvVar 'Email__AllowSmtpOnRender' (Get-Setting -Values $envValues -Name 'EMAIL_ALLOW_SMTP_ON_RENDER' -Default 'false')
    New-EnvVar 'GmailApi__ClientId' (Get-Setting -Values $envValues -Name 'GMAIL_API_CLIENT_ID' -Default (Get-Setting -Values $envValues -Name 'GOOGLE_CLIENT_ID'))
    New-EnvVar 'GmailApi__ClientSecret' (Get-Setting -Values $envValues -Name 'GMAIL_API_CLIENT_SECRET' -Default (Get-Setting -Values $envValues -Name 'GOOGLE_CLIENT_SECRET'))
    New-EnvVar 'GmailApi__RefreshToken' (Get-Setting -Values $envValues -Name 'GMAIL_API_REFRESH_TOKEN')
    New-EnvVar 'GmailApi__FromEmail' (Get-Setting -Values $envValues -Name 'GMAIL_API_FROM_EMAIL' -Default (Get-Setting -Values $envValues -Name 'SMTP_FROM_EMAIL'))
    New-EnvVar 'GmailApi__FromName' (Get-Setting -Values $envValues -Name 'GMAIL_API_FROM_NAME' -Default 'QuizHub')
    New-EnvVar 'GmailApi__TimeoutSeconds' (Get-Setting -Values $envValues -Name 'GMAIL_API_TIMEOUT_SECONDS' -Default '20')
    ...
    New-EnvVar 'Resend__TimeoutSeconds' (Get-Setting -Values $envValues -Name 'RESEND_TIMEOUT_SECONDS' -Default '20')
    New-EnvVar 'Brevo__ApiKey' (Get-Setting -Values $envValues -Name 'BREVO_API_KEY')
    New-EnvVar 'Brevo__FromEmail' (Get-Setting -Values $envValues -Name 'BREVO_FROM_EMAIL')
    New-EnvVar 'Brevo__FromName' (Get-Setting -Values $envValues -Name 'BREVO_FROM_NAME' -Default 'QuizHub')
    New-EnvVar 'Brevo__TimeoutSeconds' (Get-Setting -Values $envValues -Name 'BREVO_TIMEOUT_SECONDS' -Default '20')
    New-EnvVar 'SendGrid__ApiKey' (Get-Setting -Values $envValues -Name 'SENDGRID_API_KEY')
    New-EnvVar 'SendGrid__FromEmail' (Get-Setting -Values $envValues -Name 'SENDGRID_FROM_EMAIL')
    New-EnvVar 'SendGrid__FromName' (Get-Setting -Values $envValues -Name 'SENDGRID_FROM_NAME' -Default 'QuizHub')
    New-EnvVar 'SendGrid__TimeoutSeconds' (Get-Setting -Values $envValues -Name 'SENDGRID_TIMEOUT_SECONDS' -Default '20')
