...
if (!app.Environment.IsEnvironment("Testing"))
{
    ...
    await EnsureClassSubjectsAndChatMessagesSchemaAsync(app);
...
static async Task EnsureClassSubjectsAndChatMessagesSchemaAsync(WebApplication app)
{
    using var scope = app.Services.CreateScope();
    var logger = scope.ServiceProvider.GetRequiredService<ILoggerFactory>().CreateLogger("DatabaseClassSchema");
    ...
    try
    {
        ...
        if (provider.Contains("Npgsql", StringComparison.OrdinalIgnoreCase))
        {
            await context.Database.ExecuteSqlRawAsync("""
                ALTER TABLE "Subjects"
                    ADD COLUMN IF NOT EXISTS "OwnerId" character varying(450);

                CREATE INDEX IF NOT EXISTS "IX_Subjects_OwnerId"
                    ON "Subjects" ("OwnerId");
                ...
                CREATE INDEX IF NOT EXISTS "IX_ChatMessages_RoomType_RoomId_SentAt"
                    ON "ChatMessages" ("RoomType", "RoomId", "SentAt");
                """);
        }
        else
        {
            await context.Database.ExecuteSqlRawAsync("""
                IF COL_LENGTH(N'[Subjects]', N'OwnerId') IS NULL
                BEGIN
                    ALTER TABLE [Subjects] ADD [OwnerId] nvarchar(450) NULL;
                END;

                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_Subjects_OwnerId' AND object_id = OBJECT_ID(N'[Subjects]'))
                BEGIN
                    CREATE INDEX [IX_Subjects_OwnerId] ON [Subjects] ([OwnerId]);
                END;
                ...
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_ChatMessages_RoomType_RoomId_SentAt' AND object_id = OBJECT_ID(N'[ChatMessages]'))
                BEGIN
                    CREATE INDEX [IX_ChatMessages_RoomType_RoomId_SentAt] ON [ChatMessages] ([RoomType], [RoomId], [SentAt]);
                END;
                """);
        }
        ...
        await InsertMigrationHistoryAsync(
            context,
            provider,
            "20260622172000_AddClassSubjectsAndChatMessages",
            productVersion);
    }
    catch (Exception exception)
    {
        logger.LogError(exception, "Class subject/chat schema check failed.");
        if (app.Configuration.GetValue<bool>("Database:ContinueOnMigrationFailure"))
        {
            logger.LogWarning("Continuing startup because Database:ContinueOnMigrationFailure is enabled.");
            return;
